...
from lib.utils.utils import TrackerParams, FeatureParams, load_yaml, load_pretrain
from lib.online import dcf, TensorList, augmentation, operation, fourier
from lib.online.preprocessing import numpy_to_torch, sample_patch_multiscale, sample_patch_transformed
...
class ONLINE(object):
    ...
    def track(self, bgr_im, image, siam_tracker, siam_state) -> dict:
        ...
        siam_state['max_score']=max_score
        siam_state['score']=score_map.detach().cpu().numpy()
        siam_state['pscore']=self.siam_state['pscore']

        return siam_state,update
    def track_updater(self, bgr_im, image, siam_tracker, siam_state) -> dict:
        self.debug_info = {}
        ...
        return siam_state,flag,test_x,scale_ind,sample_pos,sample_scales,s

    def updater(self,flag,test_x,scale_ind,sample_pos,sample_scales,s):
        # ------- UPDATE ------- #
        ...
        if getattr(self.p, 'update_classifier', False):
            # print('update')
            # Get train sample
            train_x = test_x[scale_ind:scale_ind+1, ...]
