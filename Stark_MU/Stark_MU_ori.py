...
class Stark_MU_ori_Tracker(object):
    def __init__(self, image, region, p=None, groundtruth=None):
        ...
        local_state1, self.score_max, ap_dis,update = self.local_track(image)
    ...
    def tc_init(self, model_dir):
        path = '../models/Stark_MU_ori.pth.tar'
    ...
    def local_track(self, image):
        ...
        return state, max_score, ap_dis,update

    def tracking(self, image):
        self.i += 1
        local_state1, score_max, ap_dis,update = self.local_track(image)
        ...
        return [float(self.last_gt[1]), float(self.last_gt[0]), float(width),
                float(height)], iou, score_max, ap_dis,ap_dis,update
