...
from ECO_MU.Eco import Eco_Tracker
from ECO_MU.Eco_MU import Eco_MU_Tracker
from ECO_MU.Eco_MU_ori import Eco_MU_ori_Tracker
...
def run_seq_list(Dataset, p, sequence_list, data_dir,mode):
    ...
    for seq_id, video in enumerate(sequence_list):
        ...
        tic = time.time()
        if p.tracker == 'Eco_MU':
            tracker = Eco_MU_Tracker(image, region, p=p, groundtruth=groundtruth)
        elif p.tracker == 'Eco':
            tracker = Eco_Tracker(image, region, p=p, groundtruth=groundtruth)
        elif p.tracker=='Eco_MU_ori':
            tracker=Eco_MU_ori_Tracker(image, region, p=p, groundtruth=groundtruth)
        else:
            ValueError()
        ...
        all_map[0] = cv2.resize(np.squeeze(score_map), (m_shape, m_shape))
        ...
        for im_id in range(1, len(image_list)):
            ...
            region, score_map, iou, score_max, dis,lof_dis,update = tracker.tracking(image)
            # print(
