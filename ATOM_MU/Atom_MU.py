...
class ATOM_MU_Tracker(object):
    def __init__(self, image, region, p=None, groundtruth=None):
        ...
        local_state1, self.score_map, update, self.score_max, dis,lof_dis = self.local_track(image)
    ...
    def local_init(self, image, init_bbox):
        local_tracker = Tracker('atom', 'default')
    ...
    def local_track(self, image):
        ...
        if len(self.state_record) >= tcopts['time_steps']:
            ...
            # print(update.item(),'lof:',1.0/lof_dis1,'MU_score:',logits[0][0].item(),logits[0][1].item())
        else:
            update=success
    ...
    def tracking(self, image):
        self.i += 1
        local_state1, self.score_map, update, score_max, dis,lof_dis = self.local_track(image)
