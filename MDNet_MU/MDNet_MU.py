...
class MDNet_MU_Tracker(object):
    def __init__(self, image, region, imagefile=None, video=None, p=None, groundtruth=None):
        ...
        self.init_metricnet(image, init_gt1,p.lof_thresh)
        self.tc_init()
        self.lof_dis_record=[]
        self.dis_record = []
        self.state_record = []
        self.rv_record = []
        opts['pos_thresh_lof']=p.pos_thresh_lof
        opts['neg_center_thresh']=p.neg_center_thresh
        opts['similar_rate']=p.similar_rate
        # norm
    ...
    def init_metricnet(self, image, init_bbox,lof_thresh):
        ...
        while pos_examples.shape[0]==0:
            opts['overlap_pos_init'][0]-=0.1
            pos_examples = SampleGenerator('gaussian', image.size, opts['trans_pos'], opts['scale_pos'])(
                target_bbox, opts['n_pos_init'], opts['overlap_pos_init'])
        opts['overlap_pos_init'][0]=0.7
        ...
        opts['pos_rate']=lof_thresh
        opts['pos_thresh'] = self.pos_all.max() * opts['pos_rate']  # 2.5
        # opts['pos_thresh'] = opts['pos_rate']  # 2.5
        ...
        while bbreg_examples.shape[0]==0:
            opts['overlap_bbreg'][0]-=0.05
            bbreg_examples = SampleGenerator('uniform', image.size, opts['trans_bbreg'], opts['scale_bbreg'],
                                             opts['aspect_bbreg'])(
                target_bbox, opts['n_bbreg'], opts['overlap_bbreg'])
        bbreg_feats = forward_samples(self.pymodel, image, bbreg_examples, opts)
        self.bbreg = BBRegressor(image.size, overlap=opts['overlap_bbreg'])
        self.bbreg.train(bbreg_feats, bbreg_examples, target_bbox)
        opts['overlap_bbreg'][0] = 0.6
        del bbreg_feats
        torch.cuda.empty_cache()
    ...
    def tracking(self, image):
        ...
        # Save result
        region = bbreg_bbox
        self.dis_record.append((target_dist[0]-self.mean[1])/self.std[1])
        self.lof_dis_record.append((lof_dis[0]-self.mean[1])/self.std[1])
        # self.dis_record.append(1.0/(target_dist[0]+0.00001))
        # self.lof_dis_record.append(1.0 / (lof_dis[0]+0.00001))
        self.state_record.append([region[0] / width, region[1] / height,
                                  (region[0] + region[2]) / width,
                                  (region[1] + region[3]) / height])
        top_scores=top_scores.data.cpu().numpy()
        self.rv_record.append(top_scores)
        if len(self.state_record) >= self.p.start_frame:
            lof_diss=np.array(self.lof_dis_record[-tcopts["time_steps"]:]).reshape((tcopts["time_steps"], 1))
        ...
        return region,top_scores,iou,target_dist[0],lof_dis[0],success.item()
    ...
    def collect_samples_metricnet(self,image):
        ...
        while pos_examples.shape[0]==0:
            opts['overlap_pos_update'][0]-=0.1
            pos_examples = self.pos_generator(target_bbox, opts['n_pos_update'], opts['overlap_pos_update'])
        opts['overlap_pos_update'][0]=0.7
        #metric
