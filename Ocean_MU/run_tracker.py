...
from Ocean_online import Ocean_Tracker
from Ocean_online_MU import Ocean_MU_Tracker
from Ocean_online_MU_ori import Ocean_MU_ori_Tracker
...
def run_seq_list(Dataset, p, sequence_list, data_dir,mode):
    ...
    for seq_id, video in enumerate(sequence_list):
        ...
        tic = time.time()
        if p.tracker == 'Ocean':
            tracker = Ocean_Tracker(image, region, p=p, groundtruth=groundtruth)
        elif p.tracker=='Ocean_MU':
            tracker=Ocean_MU_Tracker(image, region, p=p, groundtruth=groundtruth)
        elif p.tracker=='Ocean_MU_ori':
            tracker = Ocean_MU_ori_Tracker(image, region, p=p, groundtruth=groundtruth)
        else:
            ValueError()
        score_map, score_pmap,score_max = tracker.get_first_state()
        t = time.time() - tic
        time_results = []
        ...
        all_pmap = np.zeros((num_frames, m_shape, m_shape))
        all_map[0] = cv2.resize(score_map, (m_shape, m_shape))
        all_pmap[0] = cv2.resize(score_pmap, (m_shape, m_shape))
        ...
        for im_id in range(1, len(image_list)):
            ...
            region, score_map,score_pmap,iou, score_max, ap_dis,lof_dis,update = tracker.tracking(image)
            ...
            all_map[im_id] = cv2.resize(np.squeeze(score_map), (m_shape, m_shape))
            all_pmap[im_id] = cv2.resize(np.squeeze(score_pmap), (m_shape, m_shape))
            box = np.array(
        ...
        if p.save_training_data:
            ...
            np.save(os.path.join(base_save_path, 'train_data', video + '_pmap'), all_pmap)
        if p.save_results:
            if Dataset=='got10k' and mode=='test':
