...
from Dimp_MU import Dimp_MU_Tracker
from Dimp import DiMP_Tracker
from Dimp_MU_ori import Dimp_MU_ori_Tracker
...
def run_seq_list(Dataset, p, sequence_list, data_dir,mode):
    ...
    for seq_id, video in enumerate(sequence_list):
        ...
        if p.tracker == 'Dimp_MU_ori':
            tracker = Dimp_MU_ori_Tracker(image, region, p=p, groundtruth=groundtruth)
        elif p.tracker == 'Dimp':
            tracker = DiMP_Tracker(image, region, p=p, groundtruth=groundtruth)
        elif p.tracker=='Dimp_MU':
            tracker=Dimp_MU_Tracker(image, region, p=p, groundtruth=groundtruth)
        else:
            ValueError()
