...
class STARK_ST(BaseTracker):
    def __init__(self, params, dataset_name):
        ...
        # #init the network
        # with torch.no_grad():
        #     tmp1 = np.random.rand(128,128, 3)*255
        #     tmp1=tmp1.astype(np.uint8)
        #     tmp2=np.zeros((128,128))
        #     tmp3 = self.preprocessor.process(tmp1, tmp2)
        #     _=self.network.forward_backbone(tmp3)
        #
        #     tmp1=np.random.rand(528, 1, 256)
        #     tmp1 = (Variable(torch.Tensor(tmp1))).type(torch.FloatTensor).cuda()
        #     tmp2=np.random.rand(1,528)
        #     tmp2 = (Variable(torch.Tensor(tmp2))).type(torch.bool).cuda()
        #     tmp={'feat':tmp1,'pos':tmp1,'mask':tmp2}
        #     _, _, _ = self.network.forward_transformer(seq_dict=tmp, run_box_head=True, run_cls_head=True)
    ...
    def track(self, image, info: dict = None):
        ...
        return self.state,conf_score
    def track_update(self, image, info: dict = None):
        H, W, _ = image.shape
        self.frame_id += 1
        # get the t-th search region
        x_patch_arr, resize_factor, x_amask_arr = sample_target(image, self.state, self.params.search_factor,
        ...
        conf_score = out_dict["pred_logits"].view(-1).sigmoid().item()
        return self.state,conf_score
    def update(self,image,flag,conf_score):
        # update template
        for idx, update_i in enumerate(self.update_intervals):
            if self.frame_id % update_i == 0 and flag and conf_score> 0.5:
                print(self.frame_id,'update')
                z_patch_arr, _, z_amask_arr = sample_target(image, self.state, self.params.template_factor,
                                                            output_sz=self.params.template_size)  # (x1, y1, w, h)
                template_t = self.preprocessor.process(z_patch_arr, z_amask_arr)
                with torch.no_grad():
