...
class ECO(BaseTracker):
    ...
    def track(self, image) -> dict:
        ...
        update=True
        return new_state.tolist(), score_map.cpu().data.numpy(),update
    def track_updater(self, image) -> dict:
        ...
        out = {'target_bbox': new_state.tolist()}
        return new_state.tolist(), score_map.cpu().data.numpy(),test_xf,scale_ind,sample_pos,sample_scales

    def update(self,test_xf,scale_ind,sample_pos,sample_scales):
        # ------- UPDATE ------- #
