...
class ATOM_Tracker(object):
    def __init__(self, image, region, p=None, groundtruth=None):
        ...
        local_state1, self.score_map, self.score_max, dis,lof_dis = self.local_track(image)
    ...
    def local_track(self, image):
        ...
        return state, score_map, max_score, ap_dis, lof_dis

    def tracking(self, image):
        self.i += 1
        local_state1, self.score_map, score_max, ap_dis,lof_dis = self.local_track(image)
        ...
        return [float(self.last_gt[1]), float(self.last_gt[0]), float(width),
                float(height)], self.score_map, iou, score_max, ap_dis,lof_dis
