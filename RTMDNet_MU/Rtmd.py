...
class RTMD_Tracker(object):
    ...
    def rtmdnet_track(self, image):
        ...
        return target_bbox, bbreg_bbox, top_scores.data.cpu().numpy()
    ...
    def tracking(self, image):
        self.i += 1
        target_box, reg_box, top_scores = self.rtmdnet_track(image)
        local_state = reg_box.reshape((1, 4))
        self.last_gt = [reg_box[1], reg_box[0], reg_box[1]+reg_box[3], reg_box[0]+reg_box[2]]
        with torch.no_grad():
            self.target_metric_feature_tmp = get_target_feature(self.metric_model, target_box, np.array(image))
            lof_dis,_ = lof(self.target_metric_feature_tmp.cpu().detach().numpy(), self.clf, thresh=rt_opts['pos_thresh'])
            success1, target_dist = judge_success(self.metric_model, self.target_metric_feature_tmp, self.target_metric_feature, rt_opts)
        ...
        if self.p.visualization:
            show_res(cv.cvtColor(image, cv.COLOR_RGB2BGR), np.array(self.last_gt, dtype=np.int32), '2',
                     groundtruth=self.groundtruth,score_max=0,
                     frame_id=self.i)

        return [float(self.last_gt[1]), float(self.last_gt[0]), float(width), float(height)], top_scores, iou, target_dist[0],lof_dis[0]

def eval_tracking(Dataset, video_spe=None, save=False, p=None, classes=None):
    if Dataset == "votlt":
    ...
    base_save_path = os.path.join('/home/dkn/Desktop/rtmd/rtmd_ori/', Dataset)
    ...
    for seq_id, video in enumerate(sequence_list):
        ...
        if groundtruth[0, 2] <= 9 or groundtruth[0, 3] <= 9:
            continue
        tracker = RTMD_Tracker(image, region, video=video, p=p,groundtruth=groundtruth)
        v_score = tracker.get_first_state()

        num_frames = len(image_list)
        bBoxes = np.zeros((num_frames, 11))
        bBoxes[0, :] = [box[0], box[1], box[2], box[3], 0, 1, v_score[0], v_score[1], v_score[2], v_score[3], v_score[4]]
        ...
        for im_id in range(1, len(image_list)):
            ...
            region, v_score, iou = tracker.tracking2(image)
            ...
            bBoxes[im_id, :] = [box[0], box[1], box[2], box[3], im_id, iou, v_score[0], v_score[1], v_score[2], v_score[3], v_score[4]]
            # bBoxes2[im_id, :] = region
        if save:
            np.savetxt(result_save_path, bBoxes, fmt="%.8f,%.8f,%.8f,%.8f,%d,%.8f,%.8f,%.8f,%.8f,%.8f,%.8f")
