...
from ATOM_MU.Atom_MU import ATOM_MU_Tracker
from ATOM_MU.Atom import ATOM_Tracker
from ATOM_MU.Atom_MU_ori import ATOM_MU_ori_Tracker
...
def run_seq_list(Dataset, p, sequence_list, data_dir,mode):
    ...
    for seq_id, video in enumerate(sequence_list):
        ...
        tic = time.time()
        if p.tracker == 'ATOM_MU':
            tracker = ATOM_MU_Tracker(image, region, p=p, groundtruth=groundtruth)
        elif p.tracker == 'ATOM':
            tracker = ATOM_Tracker(image, region, p=p, groundtruth=groundtruth)
        elif p.tracker=='ATOM_MU_ori':
            tracker=ATOM_MU_ori_Tracker(image, region, p=p, groundtruth=groundtruth)
        else:
            ValueError()
...
def eval_tracking(Dataset, p, mode=None):
    if Dataset == 'lasot':
        ...
        for c in classes:
            ...
            run_seq_list(Dataset, p, sequence_list, data_dir, mode=mode)
