...
class PrDimp_MU_Tracker(object):
    def __init__(self, image, region, p=None, groundtruth=None):
        ...
        local_state1, self.score_map, update, self.score_max, dis,lof_dis1 = self.local_track(image)
    ...
    def metric_init(self, im, init_box,lof_thresh):
        ...
        while gt_pos_examples.shape[0] < 10:
            ...
            print(gt_pos_examples.shape[0])
        # if gt_pos_examples.shape[0]==0:
        #     gt_pos_examples=init_box.repeat(20,0)
        # print(gt_iou)
        with torch.no_grad():
    ...
    def tc_init(self, model_dir):
        # self.tc_model = tclstm_fusion_v5()
        # path = '/media/zj/4T-2/model/ltmu/PrDimp_fusion_v4_reverse/checkpoints/ltr/meta_updater/meta_updater/tclstm_fusion_v4_ep00' + str(
        #     self.p.checkpoint) + '.pth.tar'
        path = './models/PrDiMP_MU.pth.tar'
    ...
    def local_init(self, image, init_bbox):
        local_tracker = Tracker('dimp', 'prdimp50')
    ...
    def local_track(self, image):
        ...
        if update:
            ...
            target_box = self.local_Tracker.get_iounet_box(self.local_Tracker.pos, self.local_Tracker.target_sz, sample_pos[scale_ind, :],
        ...
        return state, score_map, update, max_score, ap_dis,lof_dis1
    ...
    def tracking(self, image):
        self.i += 1
        local_state1, self.score_map, update, score_max, dis,lof_dis1 = self.local_track(image)
        ...
        return [float(self.last_gt[1]), float(self.last_gt[0]), float(width),
                float(height)], self.score_map, iou, score_max, dis,lof_dis1
