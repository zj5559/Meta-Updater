...
class Eco_Tracker(object):
    ...
    def local_track(self, image):
        state, score_map,update = self.local_Tracker.track(image)
        ...
        return state, score_map, max_score, ap_dis, lof_dis,update

    def tracking(self, image):
        self.i += 1
        local_state1, self.score_map, score_max, ap_dis,lof_dis,update = self.local_track(image)
        ...
        return [float(self.last_gt[1]), float(self.last_gt[0]), float(width),
                float(height)], self.score_map, iou, score_max, ap_dis,lof_dis,update
