...
from Stark import Stark_Tracker
from Stark_MU_ori import Stark_MU_ori_Tracker
from Stark_MU import Stark_MU_Tracker
from local_path import uav_dir,lasot_dir, tlp_dir, otb_dir, votlt19_dir, votlt18_dir,got10k_dir,tn_dir,nfs_dir,oxuva_dir
...
def rect_to_opencv(rect, imsize_hw):
    ...
    return np.array([xmin_abs, ymin_abs, xmax_abs - xmin_abs, ymax_abs - ymin_abs])
...
def get_seq_list(Dataset, mode=None, classes=None):
    ...
    if Dataset in ['votlt18', 'otb', "votlt19", "tlp", 'demo', 'lasot', 'got10k', 'oxuva']:
...
def get_groundtruth(Dataset, data_dir, video):
    if Dataset == "votlt18" or Dataset == "votlt19" or Dataset == "demo":
        ...
        sequence_dir = os.path.join(data_dir,video)
        gt_dir = os.path.join(data_dir,'..','..',video)
    if Dataset!='nfs':
        try:
...
def run_seq_list(Dataset, p, sequence_list, data_dir,mode):
    ...
    id_count=0
    if Dataset=='oxuva':
        base_save_path=os.path.join(base_save_path,mode)
        if not os.path.exists(base_save_path):
            os.makedirs(base_save_path)
        for key, task in tasks.items():
            vid, obj = key
            preds_file = os.path.join(base_save_path, '{}_{}.csv'.format(vid, obj))
            if os.path.exists(preds_file):
                continue
            image = cv2.cvtColor(cv2.imread(imfile(vid, task.init_time)), cv2.COLOR_BGR2RGB)
            h = image.shape[0]
            w = image.shape[1]
            region1 = rect_to_opencv(task.init_rect, imsize_hw=(h, w))################
            region=Region(region1[0], region1[1], region1[2], region1[3])
            groundtruth = region1[np.newaxis, :]
            tic = time.time()
            if p.tracker == 'Stark':
                tracker = Stark_Tracker(image, region, p=p, groundtruth=groundtruth)
            elif p.tracker == 'Stark_MU_ori':
                tracker = Stark_MU_ori_Tracker(image, region, p=p, groundtruth=groundtruth)
            elif p.tracker == 'Stark_MU':
                tracker = Stark_MU_Tracker(image, region, p=p, groundtruth=groundtruth)
            else:
                ValueError()
            t = time.time() - tic
            time_results = []
            ...
            for t in range(task.init_time + 1, task.last_time + 1):
                image = cv2.cvtColor(cv2.imread(imfile(vid, t)), cv2.COLOR_BGR2RGB)
                region, iou, score_max, ap_dis, lof_dis,update = tracker.tracking(image)
                present=True
                if p.tracker=='Stark':
                    if score_max<0.1:
                        present=False
                    ...
                    if not update:
                        present=False
                if present==False:
                    preds[t]=oxuva.make_prediction(present=present, score=0.0)
                else:
                    rect = rect_from_opencv(region, imsize_hw=(h, w))
                    preds[t]=oxuva.make_prediction(present=True, score=1.0, **rect)
                print(vid, obj, t-task.init_time,'/',task.last_time-task.init_time)
            tmp_preds_file = os.path.join(base_save_path, '{}_{}.csv.tmp'.format(vid, obj))
            with open(tmp_preds_file, 'w') as fp:
                oxuva.dump_predictions_csv(vid, obj, preds, fp)
            os.rename(tmp_preds_file, preds_file)
    else:

        if p.save_results and not os.path.exists(
                os.path.join(base_save_path, 'eval_results')) and not (Dataset == 'got10k' and mode=='text'):
        ...
        for seq_id, video in enumerate(sequence_list):
            ...
            if p.tracker == 'Stark':
                ...
                ValueError()
            score_max = tracker.get_first_state()
            t = time.time() - tic
            time_results = []
            # time_results.append(t)
            ...
            for im_id in range(1, len(image_list)):
                ...
                #     region, iou, score_max, ap_dis, lof_dis,update = tracker.tracking(image)
                # except:
                # region, iou, score_max, ap_dis, lof_dis = tracker.tracking(image)
                region, iou, score_max, ap_dis, lof_dis, update = tracker.tracking(image)

                # print("%d: " % seq_id + video + ": %d /" % im_id + "%d" % len(image_list)+" iou:%f"%iou+" ap_dis:%f"%ap_dis+" lof_dis:%f"%lof_dis)
                t = time.time() - tic
                time_results.append(t)
                if p.save_results and Dataset in ['votlt18', 'votlt19']:
                ...
                bBoxes_train[im_id, :] = [box[0], box[1], box[2], box[3], im_id, iou, score_max, ap_dis,lof_dis]
                bBoxes_results[im_id, :] = region
            fps = 1.0 / np.mean(np.array(time_results))
            print("%d: " % seq_id + video + ", %f" % fps)
            if p.save_training_data:
...
def eval_tracking(Dataset, p, mode=None):
    ...
    elif Dataset in ['votlt18', 'votlt19', 'tlp', 'otb', 'demo','got10k','trackingnet','uav123','nfs','oxuva']:
