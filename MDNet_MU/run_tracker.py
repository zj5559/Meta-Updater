...
from MDNet import MDNet_Tracker
from MDNet_MU import MDNet_MU_Tracker
from MDNet_MU_ori import MDNet_MU_ori_Tracker
...
def run_seq_list(Dataset, p, sequence_list, data_dir,mode):
    ...
    for seq_id, video in enumerate(sequence_list):
        ...
        tic = time.time()
        if p.tracker == 'MDNet_MU_ori':
            tracker = MDNet_MU_ori_Tracker(image, region, p=p, groundtruth=groundtruth)
        elif p.tracker == 'MDNet_MU':
            tracker = MDNet_MU_Tracker(image, region, p=p, groundtruth=groundtruth)
        elif p.tracker == 'MDNet':
            tracker = MDNet_Tracker(image, region, p=p, groundtruth=groundtruth)
        else:
            ValueError()
        ...
        for im_id in range(1, len(image_list)):
            ...
            region, scores, iou, dis,lof_dis,update = tracker.tracking(image)
            # print(
            #     "%d: " % seq_id + video + ": %d /" % im_id + "%d" % len(image_list) + " iou:%f" % iou + " dis:%f" % dis)
            t = time.time() - tic
            time_results.append(t)
            if p.save_results and Dataset in ['votlt18', 'votlt19']:
            ...
            bBoxes_train[im_id, :] = [box[0], box[1], box[2], box[3], im_id, iou, scores[0],scores[1],scores[2],scores[3],scores[4], dis,lof_dis]
